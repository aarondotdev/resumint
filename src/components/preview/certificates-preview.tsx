...
export default function CertificatesPreview({ section }: Props) {
  return (
    <div>
        ...
        <div key={entry.id} className="mb-2">
          <div className="flex justify-between items-baseline">
            <span className="text-xs font-bold text-gray-900">
              {entry.name}
              {entry.url && (
                <span className="font-bold text-teal-700"> | <span className="text-teal-600 underline">CERTIFICATE</span></span>
              )}
            </span>
            {entry.date && (
              <span className="text-xs font-bold text-teal-800">{entry.date}</span>
            )}
          </div>
